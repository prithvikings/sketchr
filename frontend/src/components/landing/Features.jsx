...
const Features = () => {
  ...
  const features = [
    {
      ...
      description: "Draw a messy circle, and we'll snap it into a perfect shape. Connect nodes instantly.",
      color: "bg-white",
      // Mini UI component for Diagramming
      visual: (
        <div className="flex items-center justify-between w-full h-24 mt-4 px-4 relative">
          <div className="w-12 h-12 bg-blue-100 border-2 border-zinc-800 rounded-lg flex items-center justify-center z-10">A</div>
          ...
          <div className="w-12 h-12 bg-green-100 border-2 border-zinc-800 rounded-full flex items-center justify-center z-10">B</div>
        </div>
      )
    },
    {
      title: "Integrated Voice Chat",
      description: "Talk to your team directly inside the board with spatial audio. No Zoom needed.",
      color: "bg-zinc-50",
      // Mini UI component for Voice Chat
      visual: (
    ...
    },
    {
      ...
      description: "Drop in Kanban boards, user journey maps, and frameworks in one single click.",
      color: "bg-zinc-50",
      // Mini UI component for Kanban Templates
      visual: (
    ...
    },
    {
      ...
      description: "Accidentally deleted the architecture diagram? Scrub back in time and restore it.",
      color: "bg-white",
      // Mini UI component for Time Travel
      visual: (
  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      mm.add("(min-width: 768px)", () => {
        ...
        // Left column slightly moves down to increase the differential gap
        tl.to(leftColumnRef.current, { y: 50, ease: "none" }, 0);

        // --- BACKGROUND PARALLAX ELEMENTS ---
        // Float 1: Large Circle moves down and scales up
        tl.to(float1Ref.current, { y: 300, scale: 1.2, rotation: 45, ease: "none" }, 0);
        // Float 2: Yellow Square moves up extremely fast and rotates
        tl.to(float2Ref.current, { y: -400, rotation: -25, ease: "none" }, 0);
        // Float 3: Small pill moves sideways and down
        tl.to(float3Ref.current, { y: 150, x: -100, rotation: 90, ease: "none" }, 0);
      });
    }, sectionRef);
  ...
  return (
    <div ref={sectionRef} className="py-24 md:py-32 px-4 md:px-8 w-full font-poppins relative overflow-hidden bg-secondarybackground mt-16">
      ...
      <div className="flex flex-col items-center text-center gap-4 mb-20 relative z-10">
        <h2 className="text-4xl md:text-5xl font-instrument text-zinc-800 tracking-tight max-w-2xl">
          Everything you need to <br className="hidden md:block" /> shape your ideas.
        </h2>
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 px-4 relative z-10">
        ...
        {/* Right Column */}
        <div ref={rightColumnRef} className="flex flex-col gap-8 md:gap-12 md:mt-48">
