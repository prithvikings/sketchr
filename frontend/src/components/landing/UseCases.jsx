...
const UseCases = () => {
  ...
  const useCases = [
    {
      ...
      description:
        "Plan sprints, build flowcharts, and align stakeholders on product roadmaps in a shared, infinite space.",
      color: "bg-blue-100",
      tags: ["Sprint Planning", "Roadmaps", "User Flows"],
      // Replaced "🗺️" with the Apple emoji image URL
      icon: map,
    },
    {
      team: "Designers",
      ...
      description:
        "Paste inspiration, leave visual feedback on UI components, and run live design critiques without screen-sharing lag.",
      color: "bg-pink-100",
      tags: ["Moodboards", "Critiques", "Wireframes"],
      // Replaced "🎨" with the Apple emoji image URL
      icon: colourpallet,
    },
    {
      team: "Engineering",
      ...
      description:
        "Draw database schemas, map out API endpoints, and troubleshoot server logic with intelligent diagramming tools.",
      color: "bg-amber-100",
  ...
  // FIX 1: Change useEffect to useLayoutEffect so GSAP layout calculations happen before the screen paints
  useLayoutEffect(() => {
        ...
        gsap.fromTo(
          ...
          },
          {
            x: () => {
              ...
              const overflow =
                cardsWrapper.scrollWidth - window.innerWidth + 64;
  ...
  return (
    <div className="border-y border-b-0 border-dashed border-zinc-400 overflow-hidden relative z-10 bg-secondarybackground">
      {/* Pinned Container */}
      {/* FIX 2: Added 'items-center' to ensure the internal layout wrapper stays dead-center regardless of pin-spacer injections */}
      <div
        ref={containerRef}
              ...
              {useCases.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-b-2 border-zinc-800/20 pb-4">
                    ...
                    <span className="flex items-center justify-center w-10 h-10 md:w-12 md:h-12 filter drop-shadow-sm">
                      <img
                        src={item.icon}
                        alt={`${item.team} icon`}
