      ...
      >
        <div className="mb-10 header-anim flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              className={`px-5 py-2 rounded-[24px] font-bold transition-all ${
                filter === "all"
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 pb-12">
            ...
            {filter === "all" && (
              <div
                onClick={() => setIsNewBoardOpen(true)} // CHANGED: Opens modal instead of firing API
            ...
            {filteredBoards.map((board) => {
              ...
              return (
                <div
                  ...
                  className={`board-item relative cursor-pointer ${getBoardColor(board._id)} border-2 border-zinc-800 rounded-[32px] p-6 h-64 flex flex-col justify-between shadow-[8px_8px_0px_#27272a] hover:shadow-[12px_12px_0px_#27272a] hover:-translate-y-2 transition-all duration-200 group`}
                >
                  {isHost && (
                    <button
                      onClick={(e) => handleDeleteBoard(e, board._id, isHost)}
                      className="absolute top-4 right-4 w-8 h-8 bg-white border-2 border-zinc-900 rounded-full flex items-center justify-center text-red-500 font-bold opacity-0 group-hover:opacity-100 hover:bg-red-100 hover:scale-110 transition-all z-10 shadow-[2px_2px_0px_#27272a]"
                      title="Delete Board"
                    >
                      ✕
                    </button>
            ...
            {filter === "shared" && filteredBoards.length === 0 && (
              <div className="col-span-full py-12 flex flex-col items-center justify-center text-center">
                <div className="w-16 h-16 bg-zinc-100 border-2 border-dashed border-zinc-300 rounded-full flex items-center justify-center text-2xl mb-4 opacity-50">
                  👻
                </div>
                <h3 className="font-instrument text-xl font-bold text-zinc-900 mb-2">
                  Nothing here yet
                </h3>
                <p className="text-zinc-500 font-poppins text-sm max-w-sm">
